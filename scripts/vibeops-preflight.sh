...
#   scripts/vibeops-preflight.sh --ci      # CI: no color; 0 In Progress OK (shipped PR / idle develop)
#   scripts/vibeops-preflight.sh --commit  # pre-commit: allow docs/tasks-only on integration branch
#
# Agent/local still requires exactly one In Progress before coding.
# CI must not: task ship marks Shipped before the PR runs, and develop is idle after merge.
set -euo pipefail
...
if [[ "$count" -eq 0 ]]; then
  if $CI; then
    # Ship marks Status=Shipped before PR CI; develop has none after merge.
    green "No In Progress TASK — OK in CI (shipped PR or idle integration)"
  elif $ship_metadata_only; then
...
if [[ "$branch" == "$production" ]] && ! $COMMIT_HOOK; then
  if $CI; then
    # Release merges land on production; CI must not treat that as local development.
    green "On $production in CI (post-release) — OK"
  else
