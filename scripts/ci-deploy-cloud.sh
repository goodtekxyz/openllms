...
echo "==> podman compose build + force-recreate ($TARGET) @ $SHA"
# Do NOT `source` the env file — values often contain shell-metacharacters.
# Compose interpolates from --env-file alone.
# Pass GIT_SHA so Dockerfile cache cannot reuse a prior COPY/. layer when only
# embedded static assets changed. Force-recreate so a new image always replaces
# a running gateway (TASK-089 hero stuck on stale embeds).
export GIT_SHA="$SHA"
podman compose -f "$COMPOSE_FILE" -p "$PROJECT" --env-file "$ENV_FILE" \
  build --build-arg "GIT_SHA=$SHA" gateway
podman compose -f "$COMPOSE_FILE" -p "$PROJECT" --env-file "$ENV_FILE" \
  up -d --force-recreate
