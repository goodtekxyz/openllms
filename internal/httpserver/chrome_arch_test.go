...
func TestChromeArchitectureSingleSource(t *testing.T) {
	...
	for _, path := range pages {
		...
		if strings.Contains(body, `id="nav-login"`) {
			t.Fatalf("%s still shows nav-login during soft-open", path)
		}
		for _, want := range []string{`class="nav"`, `id="nav-account"`, `settings-fab`, `data-theme-set`, `site-footer`} {
	...
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
	if rec.Code != http.StatusFound || !strings.Contains(rec.Header().Get("Location"), "install") {
		t.Fatalf("/login soft-open redirect got %d %s", rec.Code, rec.Header().Get("Location"))
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(tokens, want) {
			t.Fatalf("tokens.css missing shared CJK wrap rule %q", want)
		}
	}
	landingCJK := fetchAsset(t, r, "/assets/landing.css")
	if !strings.Contains(landingCJK, "html:lang(ko) .hero h1") || !strings.Contains(landingCJK, "max-width: none") {
		t.Fatal("landing.css must relax CJK hero/h2 em clamps (max-width: none)")
	}
