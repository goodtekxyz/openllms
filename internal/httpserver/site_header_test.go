...
func TestSharedHeaderAcrossHomeAndConsole(t *testing.T) {
	...
	for _, link := range []string{">홈</a>", ">API</a>", ">시작하기</a>", ">셀프 호스티드</a>"} {
		...
		if !strings.Contains(page, link) {
			t.Fatalf("page missing %s", link)
		}
	}
	if !strings.Contains(home, `id="nav-account"`) || !strings.Contains(page, `id="nav-account"`) {
		t.Fatal("home and install must share nav-account slot")
	}
	...
	if !strings.Contains(page, `href="/ko/install"`) {
		t.Fatal("install lang menu must link to /ko/install so cookie can switch back")
	}
}
